...
import { WebPageSummary } from '@chaindesk/lib/types';
            ...
            <Stack sx={{ mt: 10, width: '100%' }} spacing={2}>
              ...
              >
                {props.items?.map((each) => (
                  ...
                  >
                    <Link
                      href={`/ai-news/${slugify(
                        (each as WebPageSummary)?.output?.metadata?.title
                      )}-${each.externalId}`}
                      className="w-full"
                    >
                      <Card sx={{ width: '100%' }}>
                        <CardOverflow>
                          <AspectRatio ratio="2">
                            <img
                              src={
                                (each as WebPageSummary)?.output?.metadata
                                  ?.ogImage
                              }
                              alt=""
                            />
              ...
              <Stack sx={{ justifyContent: 'center' }} direction="row" gap={1}>
                {props.index > 0 && (
                  <Link
                    href={`/ai-news/all/${props.index - 1}`}
