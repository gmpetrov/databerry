...
const CustomPrismaProvider = (req: NextRequest) => (p: PrismaClient) => {
  ...
  return {
    ...
    createUser: async (data: Prisma.UserCreateArgs['data']) => {
      ...
      try {
        product = new URL(callbackUrl)?.searchParams?.get('product') as string;
      } catch (err) {}
...
export const authOptions = (req: NextRequest): AuthOptions => {
...
const handleGetSession = async (req: NextRequest, res: NextResponse) => {
  const headersList = headers();
