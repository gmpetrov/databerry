...
import { ChatResponse, SearchResponseSchema } from '@app/types';
...
const chat = async ({
  ...
  const qaChain = new StuffDocumentsChain({
  ...
  const chain = RetrievalQAChain.fromLLM(model as any, retriever, {
    returnSourceDocuments: true,
    inputKey: 'question',
