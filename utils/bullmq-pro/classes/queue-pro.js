...
export class QueuePro extends Queue {
  ...
  async getGroups(start = 0, end = -1) {
    ...
    const [groups, rateLimited, maxConcurrencyGroups, pausedGroups] =
      await this.scripts.getGroups(start, end);
  ...
  async getGroupsByStatus(status, start = 0, end = -1) {
    ...
    const [groupIds = [], counts = []] = await this.scripts.getGroupsByStatus(
      status,
      start,
      end
    );
  ...
  async getGroupsCount() {
    const { waiting, limited, maxed, paused } =
      await this.getGroupsCountByStatus();
  ...
  async getGroupsCountByStatus() {
    ...
    const [[err1, waiting], [err2, limited], [err3, maxed], [err4, paused]] =
      await multi.exec();
